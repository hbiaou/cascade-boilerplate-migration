...
# Tailwind CSS (replacing CDN) - using v3 for compatibility
npm install -D tailwindcss postcss autoprefixer

# Initialize Tailwind
npx tailwindcss init -p

# Install modern UI component library and utilities for professional design
npm install -D class-variance-authority clsx tailwind-merge
npm install lucide-react

# Optional: shadcn/ui can be added later if user requests professional UI components
```
...
export default {
  ...
  theme: {
    extend: {
      // Design-opinionated defaults (architect agent will customize based on user preference)
      // Note: Using / <alpha-value> syntax to support opacity modifiers (e.g., bg-primary/50)
      colors: {
...
@layer base {
  :root {
    /* Default design tokens (architect agent will customize) */
    --background: 0 0% 100%;
...
- [ ] **Design foundation installed**: CVA, clsx, tailwind-merge present in package.json
- [ ] **Tailwind config enhanced**: Custom colors, typography, spacing defined (not blank)
- [ ] **CSS variables set**: Design tokens available in index.css (:root and .dark)
- [ ] **UI library ready**: Lucide icons or equivalent installed
